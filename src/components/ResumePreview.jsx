...
const ResumeParser = () => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        if (result.errors && result.errors.length > 0) {
          setError(
            `${result.errors.length} files failed to process: ${result.errors.map((e) => e.filename).join(", ")}`,
          )
        }
  ...
  return (
    <Box maxWidth="mx" mx="auto" my={4} px={8}>
      <Typography variant="h4" align="left" mx="auto" my={4} px={0} sx={{ fontWeight: 600 }}>
        Dashboard
      </Typography>
        ...
        {activeTab === "upload" && step === 0 && (
          <>
            ...
            <Typography align="left" color="text.secondary" sx={{ mb: 4 }}>
              Supported formats PDF and Word documents. You can select multiple files.
            </Typography>
              ...
              <Table stickyHeader size="small">
                <TableHead>
                  <TableRow sx={{ bgcolor: "#f8f9fa" }}>
                    <TableCell sx={{ fontWeight: 600, width: "60px" }}>S.No</TableCell>
                ...
                <TableBody>
                  {files.map((file, index) => (
                    ...
                    >
                      ...
                      <TableCell sx={{ py: 1.5 }}>
                        <Box display="flex" alignItems="center">
                          <DescriptionOutlinedIcon sx={{ fontSize: 16, mr: 1, color: "#666" }} />
...
<Button
            ...
            <ResumeListView resumes={parsedResumes} onPreview={handlePreviewResume} onDelete={handleDeleteResume} />
          </Box>
        )}
      ...
      <ResumePreviewModal open={!!selectedResume} onClose={() => setSelectedResume(null)} resume={selectedResume} />
    </Box>
  )
}
